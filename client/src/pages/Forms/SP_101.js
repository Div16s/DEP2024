...
doc.text(`${formData.budgetHead} & ${formData.sanctionedBudget} `, 150, 27.5, { align: 'right' })
doc.setFont('times', 'bold')
...
doc.text(`${formData.readyForInstallation}`, 150, 60, { align: 'left' })

doc.setFont('times', 'bold')
doc.text(`Is Goods are required for Research Purpose: `, 21, 69, { align: 'left' })
doc.setFont('times', 'normal')
doc.line(pageWidth - 19, 70.5, 19, 70.5)
doc.text(`${formData.readyForInstallation}`, 150, 69, { align: 'left' })
doc.setFont('times', 'bold')
...
doc.text(`${formData.GEM}`, 150, 87, { align: 'left' })
doc.setFont('times', 'normal')
...
doc.text(`${formData.modeOfEnquiry}`, 150, 106, { align: 'left' })
doc.setFont('times', 'normal')
...
doc.text(`${formData.numberOfQuotation}`, 150, 111, { align: 'left' })
doc.setFont('times', 'normal')
...
doc.text(`${ formData.nameOfSupplier}`, 128.5, 116)
doc.line(124, 117.5, 172, 117.5)
doc.setFont('times', 'normal')

doc.text('against', 174, 116)
doc.text("quotation no: ", 21, 121.5)
doc.text(`${formData.quotationNumber}`, 42, 121.5)
...
formData.items.forEach((element,index,array)=>{
    flag=1
...
doc.text(`${formData.modeOfPayment}`, 80, ypos, { align: 'left' })
doc.setFont("times", 'normal')
ypos += 15
doc.setFont("times", 'bold')
doc.text("Delivery Period:", 21, ypos, { align: 'left' })
doc.text(`${formData.deliveryPeriod}`, 60, ypos, { align: 'left' })
doc.setFont("times", 'normal')
ypos += 15
doc.setFont("times", 'bold')
